...
{
    public partial class barfi : PhoneApplicationPage
    {
        private string _homeURL = "http://allrecipes.co.in/recipes/searchresults.aspx?text=barfi&o_is=SR_SrtRtng&sort=5";
        ...
        private void PinApplicationBar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ShellTile.Create(new Uri("/barfi.xaml?StartURL=" + Browser.Source.ToString(), UriKind.Relative), tileData, false);
            }
            catch (Exception ex)
            {
